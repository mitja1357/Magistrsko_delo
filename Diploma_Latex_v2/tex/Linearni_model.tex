...
B(x,y)= x
\end{equation}
Simulacijski model sestavljajo 4 Hallovi sondi postavljeni na krožnico z radijem $r_0$ in so prostorsko zamaknjene za $90^\circ$ (slika \ref{lin_simul}). Lokacija predstavlja začetno lego Hallovih sond. 
...
Z diferencialnim odčitavenjem signalov  $B_{sin}$ in $B_{cos}$, se napaka signalov odšteje. Iz tega sledi zaključek, da pri linearni apoksimaciji Z-komponente gostote magnetnega pretoka napačna montaža ne vpliva na napako.

Kljub temu so bile opravljene simulacije na poenostavljenem modelu le dveh sond
 ($H_1$ in $H_2$). Na ta način je lahko bila analizirana napaka.
...
&\delta_{sin}(\Delta x_s) =-4,71\Delta x_s^{3}-3,96\cdot 10^{-1}\Delta x_s^{2}+3,82\cdot 10\Delta x_s \\                           
...
&\delta_{sin}(\Delta y_s) =6,79\cdot 10^{-12}\Delta y_s^{3}-4,89\cdot 10^{-12}\Delta y_s^{2}+8,24\cdot 10^{-13}\Delta y_s \\
...
&\begin{split}\delta_{sin}(\Delta x_d) =6,62\cdot 10^{-12}\Delta x_d^{3}-4,89\cdot 10^{-12}\Delta x_d^{2}+8,24\cdot 10^{-13}\Delta x_d  \end{split}\\      
