...
V napaki se pojavi med $\mathrm{95^\circ}$ in $\mathrm{140^\circ}$ preskok napake. V signalih $B_{sin}$ in $B_{cos}$ se v tem območju pojavi fazni zamik, $B_{sin}$ pri $\mathrm{95^\circ}$ in nato $B_{cos}$ pri $\mathrm{160^\circ}$. Vzroka nisem raziskoval.
...
Izrazijo se tudi na napaki, ki je prikazana na sliki \ref{./MER/xs_napaka}. Med 95 in 175$^\circ$ se signala $B_{sin}$ in $B_{cos}$ hkrati fazno zamakneta. Vzrok tega pojava nisem raziskoval. Napaka razvita v Fourierovo vrsto prikaže pričakovano povišanje drugega harmonika. Povišala sta se tudi amplitudi tretjega in četrtega harmonika.
...
Iz simulacij se pričakuje hitrejše spreminjanje amplitude osnovnega harmonika $B_{cos}$ signala. Sprememba amplitud osnovnih harmonikov je prikazana na sliki \ref{./MER/xs_sincos_amp}. Amplituda signala $B_{cos}$ pada pričakovano, pada tudi ampituda signala $B_{sin}$. Signala nimata enake amplitude v izhodišču, kar je posledica neidealne izhodiščne lege. Potek enosmerne komponente je prikazan na sliki \ref{./MER/xs_sincos_off}. Komopnenta $B_{cos}$ signala pada kot je bilo predvideno v simulacijah. Fazni zamik signalov je prikazan na sliki \ref{./MER/xs_sincos_phase}. Pričakovano po simulacijah se fazna razlika med signaloma zmanjšuje. Rezultati simulacij so prikazovali konstanten fazni kot signala $B_{cos}$ in spreminjanje le faznega kota $B_{sin}$. Pri meritvah se je fazni kot $B_{cos}$ zmanjševal, fazni kot $B_{sin}$ naraščal. Razlika med njima je manjša, kot je bila posimulirana. Vsota faznih zamikov ostaja konstantna, zato se enosmerna komponenta v napaki ne spreminja.
...
\begin{eqnarray}
&A_{sin}(\Delta x_d) = -6,93            \Delta x_d^3-1,23 \Delta x_d^2-1,12 \Delta x_d+34,2               \\       
&Off_{sin}(\Delta x_d) =  5,02            \Delta x_d^3-1,75 \Delta x_d^2-15,7    \Delta x_d+9,49\cdot 10^{-2}            \\    
&\delta_{sin}(\Delta x_d) =  8,95            \Delta x_d^3-1,06 \Delta x_d^2+7,09\cdot 10^{-1}\Delta x_d-1,73\cdot 10^{-2}            \\
&A_{cos}(\Delta x_d) = -6,93            \Delta x_d^3-1,23 \Delta x_d^2-1,12 \Delta x_d+34,2                \\       
&Off_{cos}(\Delta x_d) =  5,02            \Delta x_d^3-1,75 \Delta x_d^2-15,7  \Delta x_d+9,49\cdot 10^{-2}            \\    
&\delta_{cos}(\Delta x_d) =  8,95            \Delta x_d^3-1,06 \Delta x_d^2+7,09\cdot 10^{-1}\Delta x_d-1,73\cdot 10^{-2}  
\end{eqnarray}
\begin{eqnarray}
&C_0(\Delta x_d) =7,20\Delta x_d^{3}-1,74\Delta x_d^{2}+4,85\cdot 10^{-1}\Delta x_d-1,29\cdot 10^{-2} \\                      
&C_1(\Delta x_d) =-1,69\cdot 10\Delta x_d^{3}+16,1\Delta x_d^{2}+38,1\Delta x_d-6,75\cdot 10^{-2} \\          
&C_2(\Delta x_d) =-5,14\cdot 10^{-1}\Delta x_d^{3}+12,6\Delta x_d^{2}-0,230\Delta x_d+2,97\cdot 10^{-4} \\
&C_3(\Delta x_d) =1,50\Delta x_d^{3}+6,27\Delta x_d^{2}+3,15\Delta x_d+1,10\cdot 10^{-1} \\                                   
&C_4(\Delta x_d) =-7,12\Delta x_d^{3}+14,4\Delta x_d^{2}-3,73\Delta x_d+4,02\cdot 10^{-1}
\end{eqnarray}
Poteki so podobni simulacijam brez zasukanega magneta, amplitude napake so nižje.

Pri dinamični ekscentričnosti v povzročeni smeri se signala $B_{sin}$ in $B_{cos}$ nista opazno spremenila (slika \ref{./MER/xd_sincos}). Sprememb ni niti na napaki (slika \ref{./MER/xd_napaka}). Spremembe niso opazne niti na  napaki razviti v Fourierovo vrsto (slika \ref{./MER/xd_fft}).
